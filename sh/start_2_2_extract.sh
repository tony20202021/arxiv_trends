...
# Использование: ./start_2_2_extract.sh [--interval-hours 1] [--from YYYY-MM-DD] [--to YYYY-MM-DD]
...
echo "Слежу за: backend/keywords  backend/llm  backend/storage  backend/pipeline.py  config  utils  scripts/run_scheduler.py"
echo "Ctrl+C для остановки."

exec python -m watchfiles \
    "python scripts/run_scheduler.py --step 2 --interval-hours 1 $*" \
    backend/keywords backend/llm backend/storage backend/pipeline.py config utils scripts/run_scheduler.py
